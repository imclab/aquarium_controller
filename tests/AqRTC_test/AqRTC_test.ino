...
void setup(){
  ...
  RTC.begin(DateTime(__DATE__, __TIME__)); // Set up RTC, will automatically adjust the RTC to compile time if it's not running
  if(!RTC.isRunning()){                    // Compile needs to be passed because libraries are not always recompiled
